...
# export EMBEDDING_API_VERSION=""
# export EMBEDDING_DIMENSIONS=
# export EMBEDDING_MAX_TOKENS=

async def main():
    cognee.config.set_graph_database_provider("agensgraph")
    data_directory_path = str(
        pathlib.Path(
            os.path.join(pathlib.Path(__file__).parent, ".data_storage/test_agensgraph")
        ).resolve()
    )
    cognee.config.data_root_directory(data_directory_path)
    cognee_directory_path = str(
        pathlib.Path(
            os.path.join(pathlib.Path(__file__).parent, ".cognee_system/test_agensgraph")
        ).resolve()
    )
    ...
    nodeset_text = "Agensgraph is a graph database that supports cypher."
    ...
    assert len(nodes) == 0 and len(edges) == 0, "Agensgraph graph database is not empty"
