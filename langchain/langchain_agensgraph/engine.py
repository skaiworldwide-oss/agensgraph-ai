...
``AgensGraph`` and one or more ``AgensgraphVector`` stores. This is the
production answer to the single-connection bottleneck: concurrent requests
each check out their own connection instead of serializing on one.
...
When no engine is supplied, ``AgensGraph``/``AgensgraphVector`` behave exactly
as before (a single dedicated ``psycopg`` connection).
"""
...
class AgensEngine:
    ...
    @contextmanager
    def connection(self, graph_path: Optional[str] = None) -> Iterator[psycopg.Connection]:
